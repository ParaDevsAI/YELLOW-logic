"""
download_telegram_messages_final.py

Versão FINAL do script com solução para o problema de timeout.
SOLUÇÃO: Usar context manager sem timeout explícito.
"""
import asyncio
import os
import json
...
END_DATE = datetime(2025, 8, 27, tzinfo=timezone.utc)
...
async def download_group_messages(client, group_id, group_name, target_dir):
    ...
    try:
        ...
        logger.info(f"🏁 Concluído download do grupo: {actual_group_name}")
...
async def main():
    ...
    logger.info("🚀 Iniciando Download de Mensagens do Telegram (VERSÃO FINAL)")
    ...
    # SOLUÇÃO FINAL: Usar context manager sem timeout
    logger.info("📡 Conectando ao Telegram...")
    client = TelegramClient(SESSION_NAME, int(API_ID), API_HASH)
    
    try:
        # CONEXÃO SIMPLES - SEM TIMEOUT (SOLUÇÃO PARA O PROBLEMA)
        await client.connect()
        logger.info("✅ Cliente Telegram conectado")
        
        # Verificar autorização
        if not await client.is_user_authorized():
            logger.error("❌ Usuário não autorizado! Execute reauthenticate.py primeiro")
            return
