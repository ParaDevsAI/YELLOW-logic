...
def initialize_supabase_client() -> Client:
    """
    Versão síncrona para inicializar e retornar o cliente Supabase.
    Ideal para scripts que não são baseados em asyncio.
    """
    global _supabase_client
    if _supabase_client is None:
        try:
...
async def get_all_ambassador_telegram_ids() -> set:
    """Busca de forma assíncrona todos os IDs de Telegram dos embaixadores registrados."""
    supabase = await get_supabase_client()
    if not supabase:
        return set()
    ...
    try:
        response = await asyncio.to_thread(
            supabase.table('authors').select('telegram_id').execute
        )
        if response.data:
            return {item['telegram_id'] for item in response.data}
        return set()
    except Exception as e:
        logger.error(f"Erro no Supabase ao buscar todos os IDs de embaixadores do Telegram: {e}")
        return set() 