...
async def get_all_tweet_ids_from_supabase() -> list[str]:
    """Fetches all unique tweet IDs from the Supabase tweets table."""
    supabase = await get_supabase_client()
    if not supabase:
        logger.error("Failed to get Supabase client. Cannot fetch tweet IDs.")
        return []
        
    try:
        response = await asyncio.to_thread(
            supabase.table('tweets').select('tweet_id').execute
        )
        if response.data:
            return [row['tweet_id'] for row in response.data]
        return []
    except Exception as e:
        logger.error(f"Supabase error while fetching tweet IDs: {e}")
        return []
...
async def main():
    ...
    logger.info("--- Starting Tweet Metrics Snapshot Script (Supabase Version) ---")
    
    api_key = os.getenv("TWITTER_API_KEY")
    if not api_key:
        logger.critical("TWITTER_API_KEY not found. Aborting.")
        return

    all_tweet_ids = await get_all_tweet_ids_from_supabase()
    if not all_tweet_ids:
        logger.warning("No tweets found in the database to update. Exiting.")
        return

    logger.info(f"Found {len(all_tweet_ids)} unique tweets to process.")
    ...
    logger.info("--- Tweet Metrics Snapshot Script Finished ---")
